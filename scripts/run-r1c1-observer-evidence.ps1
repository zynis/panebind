...
param(
    ...
    [string] $BuildDirectory = 'out/r1c1-debug',
...
function Resolve-R1C1Executable {
    param(
        [Parameter(Mandatory = $true)]
        [string] $FileName
    )
...
$observerPath = Resolve-R1C1Executable -FileName 'panebind-observer.exe'
$harnessPath = Resolve-R1C1Executable -FileName 'panebind-companion-harness.exe'

$evidenceDirectory = Join-Path -Path $repositoryRoot -ChildPath 'uat/r1c1'
...
try {
    ...
    # companion harness launches the PaneBind-owned target process and windows.
...
if ($observerExitCode -ne 0 -or $harnessExitCode -ne 0) {
    throw "R1-C1 evidence processes failed: observer=$observerExitCode, harness=$harnessExitCode"
}
