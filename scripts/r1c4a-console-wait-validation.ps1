function Assert-C4AConsoleWaitEvidence {
    ...
    Assert-C4A ($marked -and $startup.console_wait_contract -ceq 'sta_message_pump_v1' -and $startup.console_input_contract -ceq 'readconsoleinputex_nowait_v1' -and $startup.owner_sta_thread -gt 0) 'STA console wait contract missing/wrong'
    ...
    foreach($wait in $waits) {
        Assert-C4A ($wait.input_wait_kind -cin $kinds -and $wait.wait_result -ceq 'complete' -and $wait.console_mode_restored -eq $true -and $wait.error -eq 0) 'console wait failed or modes not restored'
        ...
        foreach($field in @('characters','text','line','key_code','scan_code','message_contents')) {
