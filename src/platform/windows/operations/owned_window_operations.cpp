...
TranslationBridgeResult bridge_visible_translation(
    ...
    if (!positioning_width.has_value() || !positioning_height.has_value() ||
        ...
        return {TranslationBridgeStatus::ArithmeticOverflow, 0, 0, std::nullopt};
    }
    if (*positioning_width == 0 || *positioning_height == 0 ||
        ...
        *target_visible_width == 0 || *target_visible_height == 0) {
        return {TranslationBridgeStatus::EmptyGeometry, 0, 0, std::nullopt};
    }
    if (*current_visible_width != *target_visible_width ||
        *current_visible_height != *target_visible_height) {
        return {TranslationBridgeStatus::ResizeRejected, 0, 0, std::nullopt};
    }
    ...
    if (!native_representable(*target_left) ||
        ...
        return {TranslationBridgeStatus::NativeCoordinateOutOfRange,
                0,
                0,
                std::nullopt};
    }

    return {TranslationBridgeStatus::Succeeded,
            *left_delta,
            *top_delta,
